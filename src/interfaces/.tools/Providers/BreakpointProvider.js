  ...
  useEffect(() => {
    ...
    if (window && window.matchMedia) {
      ...
      keys.forEach(media => {
        ...
        } else matches[media] = false;
      });
      setQueryMatch(matches);
      isAttached = true;
      keys.forEach(media => {
        if (typeof queries[media] === 'string') mediaQueryLists[media].addListener(handleQueryListener);
      });
    }
    ...
    return () => {
      if (isAttached) {
        keys.forEach(media => {
          if (typeof queries[media] === 'string') mediaQueryLists[media].removeListener(handleQueryListener);
        });
      }
    };
  }, [queries]);
