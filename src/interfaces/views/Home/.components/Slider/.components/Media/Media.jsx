  ...
  render() {
    ...
    if (source) {
      if (source.match(/\.(mp4|webm)/)) {
        background = <video title={media.title || media[`data-title`]} src={source} type="video/mp4" controls />;
      } else {
        // DEFAULTS TO AN IMAGE TAG
        background = <img alt={media.alt || media.title || media[`data-alt`] || null} src={source} />;
      }
    }
