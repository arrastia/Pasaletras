...
export class Bullets extends React.Component {
  ...
  renderBullets() {
    ...
    return media.map((item, index) => {
      ...
      return (
        <button key={`bullet-${index}`} data-index={index} onClick={this.bulletClick} className={className}>
          {index}
        </button>
      );
    });
  ...
  render() {
    ...
    return <nav className={getClassName(`${rootElement}__bullets`, cssModule)}>{this.renderBullets()}</nav>;
  }
}
