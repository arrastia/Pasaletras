  ...
  const onTabDragOver = event => {
    if (!isUndefined(initialTabIndexDrag)) {
      ...
      if (event.currentTarget.tabIndex !== initialTabIndexDrag) {
        if (!isDragging) {
          if ((index === '-1' && totalTabs - initialTabIndexDrag !== 1) || (index !== '-1' && initialTabIndexDrag - index !== -1)) {
            setIsDragging(true);
          }
        }
      }
    ...
    <React.Fragment>
        ...
        style={{
          ...
          left:
            !isUndefined(tabRef.current) && !isUndefined(divScrollTabsRef)
              ...
              : '0px',
          position: 'absolute',
          top: !isUndefined(tabRef.current) ? `${tabRef.current.offsetTop + tabRef.current.clientHeight - 4}px` : '100px',
          zIndex: 9999
        }}>
      ...
      <li
        ...
        <a
          ...
          className={editable ? styles.p_tabview_design : addTab ? styles.p_tabview_design_add : styles.p_tabview_noDesign}
          href={'#' + ariaControls}
          id={id}
